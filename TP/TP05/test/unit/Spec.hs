...
main :: IO ()
main = defaultMain (testGroup "TP05" tests)
  ...
  [ testGroup
    ...
    testGroup
      ...
      [ testProperty "length'" ((\xs -> length xs == length' xs) :: [Int] -> Bool),
        ...
        testCase "concat' infinite list" (take 10 (cycle [1, 2]) @?= take 10 (concat' (repeat [1, 2]))),
        ...
        testProperty "elem' chars" ((\c xs -> elem' c xs == elem c xs) :: Char -> String -> Bool),
        testProperty "elem' infinite list" ((\x -> x > 0 ==> elem' x [1 ..]) :: Int -> Property),
        testProperty "notElem' chars" ((\c xs -> notElem' c xs == notElem c xs) :: Char -> String -> Bool),
        testProperty "find' chars" ((\c xs -> find' (== c) xs == find (== c) xs) :: Char -> String -> Bool),
